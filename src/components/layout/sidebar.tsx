      ...
      >
        <div className="flex items-center h-14 px-4 border-b border-[var(--glass-border)] flex-shrink-0 justify-between">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-md bg-[#1856FF]/20 flex items-center justify-center flex-shrink-0">
            ...
            <div className="space-y-0.5">
              {vault.chapters.slice(0, 6).map((ch) => (
                <Link key={ch.name} href={`/reader/${encodeURIComponent(ch.name)}`}
                  className="block px-3 py-1 rounded text-xs opacity-40 hover:opacity-70 hover:bg-white/[0.03] transition-colors truncate">
                  {ch.name}
                </Link>
              ))}
            </div>
